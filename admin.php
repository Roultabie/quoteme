...
if (is_array($quotes)) {
    foreach ($quotes as $quote) {
        ...
        if (!empty($userConfig['shaarli'])) {
            $html->setElement('shaarli', '<a class="icon-shaarli" href="' . rtrim($userConfig['shaarli'], '/') . '/?post=http://' . $_SERVER['HTTP_HOST'] . '/?' . $quote->getPermalink() . '&source=bookmarklet" onclick="javascript:window.open(this.href,\'\',\'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600\');return false;">Shaarli</a>', 'quoteTable');
        }
